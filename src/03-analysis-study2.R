...
kw1 <- read.table('../input/dict-2.txt', stringsAsFactors = F, sep=',', header = T) %>% rename(TARGET = word)
kw2 <- read.table('../input/descriptive_terms.txt', stringsAsFactors = F, sep=',', header = T) %>% rename(TARGET = word)
